...
TableView.prototype.onItemsChanged = function(lastCurrentItem) {
    var start = 0;
    if (lastCurrentItem) {
...
TableView.prototype.displayItems = function(start) {
    ...
    var itemsCount = this.itemsCollection.getItemsCount();
    var end = Math.min(this.tableCapacity + 1 + start, itemsCount);
    for (var i = start; i < end; i++) {
       this.insertItem(i);
    }
    this.scrollbar.setItemsCount(itemsCount, this.tableCapacity);
};
...
TableView.prototype.upOne = function() {
    ...
    if (this.currentItem - this.startPosition < 0) {
        if (this.currentItem + this.tableCapacity < this.itemsCollection.getItemsCount() - 1)
...
TableView.prototype.downOne = function() {
    if (this.currentItem == this.itemsCollection.getItemsCount() - 1)
        return;
...
TableView.prototype.pageDown = function() {
    ...
    if (this.currentItem - this.startPosition < this.tableCapacity - 1) {
        this.currentItem = Math.min(this.tableCapacity, this.itemsCollection.getItemsCount()) - 1 + this.startPosition;
        if (this.currentItem >= this.itemsCollection.getItemsCount())
            this.currentItem = this.itemsCollection.getItemsCount() - 1;
    }
    else {
        this.currentItem += this.tableCapacity;
        if (this.currentItem >= this.itemsCollection.getItemsCount())
...
TableView.prototype.onResize = function(newWidth, newHeight) {
    ...
    if (!this.itemsCollection.items || this.itemsCollection.getItemsCount() == 0) {
        return;
    }
    ...
    this.resizeScrollUpTimerInterval = setInterval(function() {
        ...
        if (self.itemsCollection.getItemsCount() - start < self.tableCapacity)
            start = self.itemsCollection.getItemsCount() - self.tableCapacity;
        if (start < 0)
            start = 0;
...
TableView.prototype.refreshItems = function() {
    var self = this;
    $(this.tableSelector + ' tr').each(function(){
        self.itemsCollection.refreshItem($(this));
    });
};
...
TableView.prototype.initializeRowHeight = function() {
    ...
    this.clearItems();
    this.setClipRegion(this.$tableView.data("width"), this.$tableView.data("height"));
};

TableView.prototype.calculateTableHeight = function() {
    this.tableCapacity = Math.floor((this.$tableView.data("height") - this.$thead.height()) / this.rowHeight) - 1;
    this.scrollbar.setItemsCount(undefined, this.tableCapacity);
};
